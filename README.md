...
- **Working label**: the line names what the running tool is doing — `Reading src/index.ts`, `Running git`, `Searching for TODO` — and the shimmer sweeps whatever it says. omp gets that sentence from the model, adding a field to every tool's schema and asking for the intent of each call; this reads it off the arguments pi already provides, which costs no tokens and needs nothing of the model. Set `"derivedLabels": false` in `~/.pi/agent/pi-omp-feel.json` for a plain `Working…`. A label another extension hands over (see below) still outranks it.
...
| `activity` | what the working line says while this tool runs — `Reading index.ts`, `Running git` |
